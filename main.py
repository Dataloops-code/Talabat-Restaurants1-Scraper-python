...
class MainScraper:
    ...
    async def run(self):
        ...
        for idx, (area_name, area_url) in enumerate(ahmadi_areas):
            if area_name == "الرقه":
                print(f"Resetting data for {area_name} to start from page 1")
                logging.info(f"Resetting data for {area_name}")
                if self.current_progress["current_progress"]["area_name"] == "الرقه":
                ...
                if "الرقه" in self.current_progress["completed_areas"]:
                    self.current_progress["completed_areas"].remove("الرقه")
                if self.scraped_progress["current_progress"]["area_name"] == "الرقه":
                ...
                if "الرقه" in self.scraped_progress["completed_areas"]:
                    self.scraped_progress["completed_areas"].remove("الرقه")
                if "الرقه" in self.scraped_progress["all_results"]:
                    del self.scraped_progress["all_results"]["الرقه"]
                for file in [os.path.join(self.output_dir, "الرقه.xlsx"), os.path.join(self.output_dir, "الرقه_detailed.xlsx")]:
                ...
                self.save_current_progress()
                self.save_scraped_progress()
                self.commit_progress(f"Reset data for الرقه to start from page 1")
                for file in [os.path.join(self.output_dir, "الرقه.xlsx"), os.path.join(self.output_dir, "الرقه_detailed.xlsx")]:
                    if os.path.exists(file):
                        print(f"Warning: {file} still exists after reset")
                        logging.warning(f"{file} still exists after reset")
                    else:
                        print(f"Verified {file} does not exist")
                        logging.info(f"Verified {file} does not exist")
                if current_area_index > idx:
